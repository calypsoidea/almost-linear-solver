...
function run() {
    ...
    const pools = { 
        ...
        2: { token0:"C", token1:"A", reserve0:1e6, reserve1:1.5e6 }  
        // ← More C than A → A is expensive in this pool → profitable to SELL A here (i.e. buy C cheap)
    };

    // Keep the original sign (or whatever you have now — doesn't matter anymore)
    ...
    for (const cyc of cycles) {
        ...
        if (optimalProfit > 0) {
            console.log("\nARB FOUND (best direction selected automatically)");
            bestTrace.forEach(t =>
                console.log(
                    `${t.pool}: ${t.from}→${t.to} | ` +
            ...
            console.log("Cycle found but no positive profit in either direction.");
        }
    }
}
