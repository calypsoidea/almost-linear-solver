...
function run() {
    ...
    2: { token0:"C", token1:"A", reserve0:1e6, reserve1:1.05e6 }  // slight imbalance
};
...
// infinitesimal rates ONLY for cycle detection
    ...
    for (const cyc of cycles) {
        const ordered = orderCycle(cyc);
        if (!validateCycle(ordered)) continue;

        const { optimalInput, optimalProfit } =
            findOptimalInput({
                ordered,
                pools,
                minIn: 1,
        ...
        if (optimalProfit <= 0) return;

        const { trace } =
            simulateCycle(ordered, pools, optimalInput);
        ...
        if (optimalProfit > 0) {
            ...
            trace.forEach(t =>
                console.log(
                    `${t.pool}: ${t.from}→${t.to} | ` +
                    `${t.in.toFixed(4)} → ${t.out.toFixed(4)}`
                )
            );

            console.log("OPTIMAL INPUT:", optimalInput.toFixed(6));
            console.log("OPTIMAL PROFIT:", optimalProfit.toFixed(6));
