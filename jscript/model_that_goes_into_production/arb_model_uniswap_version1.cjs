...
function gaugeFix(c) {
    ...
    return c.map(x => x - mean);
}

function liquidityBound(ordered, pools, frac = 0.02) {
    let bound = Infinity;

    for (const t of ordered) {
        const p = pools[t.eid];
        const rin = (t.from === p.token0) ? p.reserve0 : p.reserve1;
        bound = Math.min(bound, rin * frac);
    }
...
function simulateCycleWithMutation(ordered, pools, inputAmount) {
    ...
    // clone pools so we don't mutate truth
    const localPools = structuredClone(pools);

    for (const t of ordered) {
        const p = localPools[t.eid];

        let rin, rout, inKey, outKey;
...
function cycleStrength(cycle, c) {
    return cycle.reduce((s, t) => s + Math.abs(c[t.eid]), 0);
}
...
function run(preferredToken = null) {
    ...
    const pools = { 
        ...
        3: { token0:"HSK", token1:"USDT", reserve0:1e6, reserve1:8e6 }
    }; 
    ...
    const c_raw = projectToCycleSpace(idToEdge, g);
    const c = gaugeFix(c_raw);
    ...
    const MIN_CYCLE_STRENGTH = 1e-6; // tune later

    for (const cyc of cycles) {

        if (cycleStrength(cyc, c) < MIN_CYCLE_STRENGTH) {
            continue;
        }
        ...
        const maxIn = liquidityBound(ordered, pools, 0.02);
        ...
        if (validateCycle(reversed)) {
            ...
            const revMaxIn = liquidityBound(reversed, pools, 0.02);
        ...
        // ===== EXECUTION-REALITY CHECK (reserve mutation) =====
        const mutatedResult = simulateCycleWithMutation(
            bestOrdered,
            pools,
            optimalInput
        );

        if (mutatedResult.profit <= 0) {
            // False positive: static math says profit, execution says loss
            continue;
        }

// Replace trace with execution-real trace
bestTrace = mutatedResult.trace;
optimalProfit = mutatedResult.profit;
