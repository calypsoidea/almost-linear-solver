...
def build_tree_A(edges, nodes, root):
    """
    Build sparse A as dict: A_cols[v] = list of (eid, coeff)
    Tree is BFS tree on undirected version (we route along directed edges if available)
    """
    G = nx.DiGraph()
    G.add_nodes_from(nodes)
    G.add_edges_from(edges)
    # BFS tree on underlying undirected graph
    U = nx.Graph()
    U.add_nodes_from(nodes)
    U.add_edges_from(edges)
    if root not in nodes:
        raise ValueError("root not in nodes")
    T = nx.bfs_tree(U, root)
    # map edges -> eid
    edge_to_id, id_to_edge = build_edge_index(edges)
    # for each v build column
    A_cols = {}
    for v in nodes:
        if v == root:
            A_cols[v] = []
            continue
        # path from v to root in tree (list of vertices)
        path = nx.shortest_path(T, source=v, target=root)
        col = {}
        for i in range(len(path)-1):
            a = path[i]
            b = path[i+1]
            # try match directed edge (a->b)
            if (a,b) in edge_to_id:
                eid = edge_to_id[(a,b)]
                col[eid] = col.get(eid, 0.0) + 1.0
            elif (b,a) in edge_to_id:
                eid = edge_to_id[(b,a)]
                # path uses reverse of original directed edge -> put -1
                col[eid] = col.get(eid, 0.0) - 1.0
            else:
                # shouldn't happen if original graph is strongly connected wrt path
                raise RuntimeError(f"No directed edge for tree step {a}->{b}")
        A_cols[v] = list(col.items())
    return A_cols
